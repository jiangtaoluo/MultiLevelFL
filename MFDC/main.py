...
for i in range(400):
    ...
    for j in range(25):
        ...
        if br.predict(ran_car_new) <= 50 and ran_car[1] <= 50:
            pre_choice_car += 1
        if ran_car[1] <= 50:
            true_car += 1
    if true_car == 0:
        continue
    pre_success = pre_choice_car / true_car
    ran_success = true_car / 25
    print(pre_success, ran_success)
...
for _ep in range(MAX_EPISODES):
    print('EPISODE :- ', _ep)
    pre_time = time.time()
    ep_reward, ep_hit, delay = 0, 0, 0
    Qw_locals = []  # 网络权重
    for bsNum in range(BS_NUM):
        # global_content_popular = (np.array(local1_content_popular) + np.array(local2_content_popular) + np.array(neibor_content_popular)).tolist()
        for worker in range(2):
            ...
            for i in range(len(s_r_list)):
                ...
                if s_c[bsNum][f_index] == 1:
                    ...
                    step_Qloss, Qw = trainer[2 * bsNum + worker].optimize()
                    ...
                    if C_BS[bsNum] - 1 >= 0:
                        ...
                        state = s_r + s_c[bsNum] + neibor_cachestate + content_popu_list
                        s_c[bsNum][f_index] = 1
                        action = trainer[2 * bsNum + worker].get_exploitation_action(state, f_index, neibor_cachestate)
                        if action[1] == 1:
    ...
    Qw_glob = FedAvg(Qw_locals)
    # copy weight to net_glob
    for w in range(args.num_users * BS_NUM):
        qnet[w].load_state_dict(Qw_glob)
    if _ep % 5 == 0:
        for w in range(args.num_users * BS_NUM):
...
torch.save(obj=Qnet_glob.state_dict(), f="net-cocarl.pth")
name = ['Reward']
test1 = pd.DataFrame(columns=name, data=Reward)
# print(test)
test1.to_csv('reward-cocarl.csv', encoding='gbk')

name = ['Loss']
test2 = pd.DataFrame(columns=name, data=Qloss)
# print(test)
test2.to_csv('loss-cocarl.csv', encoding='gbk')

# plt.figure()
# plt.subplot(2, 1, 1)
# plt.xlabel('ep')
# plt.ylabel('Q_loss')
# plt.plot(range(len(Qloss)), Qloss)
